...
- The coordinator also sees the **heatwave advisory** for their ward with severity level, do/don't lists, and best outdoor timing.

---

#### Step 12 — Heatwave Advisory (AI)

**On the coordinator or resident dashboard, tap the heatwave banner**

- The full heatwave advisory appears with:
  - **Severity**: watch, warning, or emergency
  - **Advisory text**: plain-English safety instructions written at a Class 5 reading level
  - **Do list**: 3 specific actions to take
  - **Don't list**: 2 things to avoid
  - **Best time outdoors**: safest window for outdoor activity
- This advisory was generated by Groq AI based on live weather data from OpenWeatherMap.
- The advisory is created automatically by the heatwave-check cron job running every 6 hours.
...
#### Step 15 — Nudge System

**Login as `resident@test.com`**

- The nudge card on the home screen shows a personalized message about water running low.
- Tap "Mark as Read" to dismiss it.
- The nudge system has two layers:
  1. **AI-powered predictive nudges** (Agent 6): Generated daily by Groq based on booking history and heatwave conditions. More personalized and contextual.
  2. **Rule-based nudges** (Agent 8): Deterministic fallback that sends reminders when estimated days remaining falls below a threshold. No AI involved — purely formula-based.
- Both write to the same `nudge_log` table with `type: 'predictive'` or `type: 'rule_based'`.
